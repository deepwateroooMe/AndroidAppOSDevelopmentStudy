...
- 目前比较感兴趣的版块包换： *android SDK, android module builds, NDK/jni, openGL es的原理与应用* 希望一一捡回来（借助今天的一个相机小项目，以后就不会再对相机的原生开发感觉陌生了！！！）
...
- todo: 主要剩下的问题还包括：
  - texworks的中文配置，以及mint包，现在还没有着手做，但从今天往后要着手解决这个问题了（发了几天疯(被我从.emacs.d recompiled whole directory......)的emacs这两天、今天终于算是正常一点儿了）
  - emacs中字体出不来像板砖的问题，其实只要换一下字体就解决了，现在的中英文字体都很好看，只有一个小问题：还没有办法设置默认字体Incolada-dz for powerline,必须每次手动设置，以后会解决；
  - 被我因先前的电脑看不清楚把org-mode org-level弄混了，现在的显示好一点儿，需要把之前弄坏的org-level校正过来
